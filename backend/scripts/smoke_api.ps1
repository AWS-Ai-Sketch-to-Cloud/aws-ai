...
$patched = Invoke-RestMethod -Method Patch -Uri "$BaseUrl/api/sessions/$sessionId/status" -ContentType "application/json" -Body (@{
    status = "ANALYZING"
    errorCode = $null
    errorMessage = $null
} | ConvertTo-Json)
Write-Host "patchedStatus=$($patched.status)"
...
$arch = Invoke-RestMethod -Method Post -Uri "$BaseUrl/api/sessions/$sessionId/architecture" -ContentType "application/json" -Body $archBody
Write-Host "architectureStatus=$($arch.status)"
